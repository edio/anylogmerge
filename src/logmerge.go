...
func postInit() {
	...
	inScanners = make([]*bufio.Scanner, len(inFileNames))
	for index, name := range inFileNames {
		file := openFile(name)
		inFiles[index] = file
		inScanners[index] = bufio.NewScanner(file)
	}
...
func main() {
	...
	//	var entries []Entry = make([]Entry, len(inScanners))
	for len(inScanners) > 0 {
		for index, scanner := range inScanners {
			s, eof := readLine(scanner)
			if !eof {
				fmt.Fprintln(outFile, s)
			} else {
				inScanners[index] = nil
			}

		}
		inScanners = inScanners[1:]
	}

	return
}

func readLine(scanner *bufio.Scanner) (s string, eof bool) {
	if scanner.Scan() {
		s = scanner.Text()
